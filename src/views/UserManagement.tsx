...
interface User {
  ...
  email: string;
  roleIds: string[];
  groupIds: string[]; // Added: Link user to groups
  branch: string;
...
export default function UserManagement() {
  ...
  // Add 'User Groups' to available tabs
  const tabs = ['All Users', 'Roles & Rights', 'User Groups', 'User Leaves', 'Audit Log'];
  ...
  const [userGroups, setUserGroups] = useState<UserGroup[]>([]); // Added
  const [permissions, setPermissions] = useState<Permission[]>([]);
  const [leaves, setLeaves] = useState<UserLeave[]>([]);
  const [auditLogs, setAuditLogs] = useState<AuditLog[]>([]);
  
  const [isCreatingRole, setIsCreatingRole] = useState(false);
  const [isCreatingGroup, setIsCreatingGroup] = useState(false); // Added
  ...
  const [newGroupDesc, setNewGroupDesc] = useState(''); // Added
  ...
  const [editingGroup, setEditingGroup] = useState<UserGroup | null>(null); // Added
  ...
  const handleDeleteGroup = async (groupId: string) => {
    if (!confirm('Are you sure you want to retire this user group?')) return;
    try {
      await deleteDoc(doc(db, 'userGroups', groupId));
    } catch (error) {
      handleFirestoreError(error, OperationType.DELETE, 'userGroups');
    }
  };
  ...
  useEffect(() => {
    ...
    const unsubGroups = onSnapshot(collection(db, 'userGroups'), (snap) => {
      setUserGroups(snap.docs.map(doc => ({ id: doc.id, ...doc.data() }) as UserGroup));
    });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between p-6 bg-white border border-[#E2E8F0] rounded-[32px] mb-6 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-[#64748B]">Manage users and their roles</p>
          </div>
        </div>
        <button 
          onClick={() => setIsCreatingUser(true)}
          className="flex items-center gap-2 bg-[#0F172A] text-white px-6 py-4 rounded-2xl font-bold text-xs uppercase tracking-widest hover:bg-[#1E293B] shadow-sm transition-all"
        >
          <Plus size={16} />
          Add User
        </button>
      ...
      <div className="flex gap-6">
        <div className="flex-1 bg-white p-8 rounded-[32px] border border-[#E2E8F0] shadow-sm">
          <DataTable 
             columns={userColumns} 
             data={filteredUsers}
          />
        ...
        <div className="w-[380px] space-y-6">
           <div className="bg-[#0F172A] text-white p-8 rounded-[32px] shadow-lg">
             <div className="flex items-center gap-4 mb-6">
               ...
               <div>
                 ...
                 <p className="text-sm text-[#94A3B8]">All Rights</p>
               </div>
             </div>
             <div className="bg-[#1E293B] p-6 rounded-2xl flex items-start gap-4">
               <div className="mt-1 text-[#D4AF37]">
                 <ShieldCheck size={20} />
               </div>
               <p className="text-sm text-white/90">
                 Admin has full access to all modules, users and settings.
               </p>
             </div>
           </div>

           <div className="bg-white p-8 rounded-[32px] border border-[#E2E8F0] shadow-sm">
             <h4 className="font-bold text-[#0F172A] mb-6">Role & Permission Overview</h4>
             <div className="space-y-4">
               {roles.map(role => (
                 <div key={role.id} className="flex items-center gap-4 p-4 hover:bg-[#F8FAFC] rounded-2xl transition-all">
                   <div className="bg-[#F1F5F9] p-2 rounded-xl text-[#0F172A]">
                     <Shield size={16} />
                   </div>
                   <div>
                     <p className="text-sm font-bold text-[#0F172A]">{role.name}</p>
                     <p className="text-xs text-[#64748B]">{role.description || 'Limited scope'}</p>
                   </div>
                 </div>
               ))}
             </div>
      ...
      <AnimatePresence>
        {isCreatingGroup && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-[#0F172A]/40 backdrop-blur-sm">
            ...
            >
              <h3 className="text-xl font-bold mb-6">{editingGroup ? 'Edit Group' : 'Create New Group'}</h3>
              <form onSubmit={handleCreateGroup} className="space-y-4">
                <input 
                  ...
                  className="w-full p-4 border border-[#E2E8F0] rounded-2xl outline-none focus:border-[#D4AF37]"
                  required
                />
                <textarea 
                  value={newGroupDesc} 
                ...
                <div className="flex gap-3 mt-8">
                  <button type="button" onClick={() => setIsCreatingGroup(false)} className="flex-1 py-4 bg-[#F8FAFC] text-[#64748B] rounded-2xl font-bold text-xs uppercase tracking-widest hover:bg-[#F1F5F9] transition-all">Cancel</button>
                  <button type="submit" className="flex-1 py-4 bg-[#0F172A] text-white rounded-2xl font-bold text-xs uppercase tracking-widest hover:bg-[#1E293B] shadow-lg transition-all">Save</button>
                </div>
              </form>
            </motion.div>
          </div>
        ...
        {roles
          ...
          >
            <UserGroups 
              ...
              onEditGroup={(group) => { setEditingGroup(group); setNewGroupName(group.name); setNewGroupDesc(group.description || ''); setIsCreatingGroup(true); }}
              onCreateGroup={() => { setEditingGroup(null); setNewGroupName(''); setNewGroupDesc(''); setIsCreatingGroup(true); }}
